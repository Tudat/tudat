...
BOOST_AUTO_TEST_CASE( test_json_acceleration_sphericalHarmonicGravity )
{
         ...
         linkEndIterator != linkEndsPerObservable.end( ); linkEndIterator++ )
    {
        ...
        for( unsigned int i = 0; i < currentLinkEndsList.size( ); i++ )
        {
            ...
            {
                currentObservationSettings = boost::make_shared< OneWayDifferencedRangeRateObservationSettings >(
            ...
            else if( currentObservable == n_way_range )
            {
                ...
                else if( i == 1 )
                {
                    ...
                    oneWayRangeObsevationSettings.push_back(
                                boost::make_shared< ObservationSettings >(
                                    one_way_range, nullptr, boost::make_shared< ConstantObservationBiasSettings >(
                                        Eigen::Vector1d::Constant( 1.0 ), true ) ) );
