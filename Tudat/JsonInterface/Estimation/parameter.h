...
void to_json( nlohmann::json& jsonObject, const boost::shared_ptr< EstimatableParameterSettings >& parameterSettings );

//! Create a shared pointer to a `EstimatableParameterSettings` object from a `json` object.
void from_json( const nlohmann::json& jsonObject, boost::shared_ptr< EstimatableParameterSettings >& parameterSettings );
