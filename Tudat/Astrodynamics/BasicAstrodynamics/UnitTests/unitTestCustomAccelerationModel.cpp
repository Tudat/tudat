...
BOOST_AUTO_TEST_CASE( test_customAccelerationModelCreation )
{
    ...
    double sourceBodyRadius = bodyMap.at( "Sun" )->getShapeModel( )->getAverageRadius( );
    double occultingBodyRadius = bodyMap.at( "Earth" )->getShapeModel( )->getAverageRadius( );

    for( auto it : dependentVariableHistory )
    {
        double shadowFunction =
                mission_geometry::computeShadowFunction(
                                    bodyMap.at( "Sun" )->getStateInBaseFrameFromEphemeris( it.first ).segment( 0, 3 ), sourceBodyRadius,
                                    bodyMap.at( "Earth" )->getStateInBaseFrameFromEphemeris( it.first ).segment( 0, 3 ), occultingBodyRadius,
                                    stateHistory.at( it.first ).segment( 0, 3 ) +
                    bodyMap.at( "Earth" )->getStateInBaseFrameFromEphemeris( it.first ).segment( 0, 3 ) );
