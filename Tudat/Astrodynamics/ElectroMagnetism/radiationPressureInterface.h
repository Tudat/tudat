...
public:
    ...
    PanelledRadiationPressureInterface(
            ...
            const boost::function< Eigen::Vector3d( ) > sourcePositionFunction,
            const boost::function< Eigen::Vector3d( ) > targetPositionFunction,
            const std::vector< boost::function< Eigen::Vector3d( ) > > localFrameSurfaceNormals,
            const std::vector< double > emmisivities,
            const std::vector< double > areas,
            const std::vector< double > diffusionCoefficients,
            const boost::function< Eigen::Quaterniond( ) > rotationFromLocalToPropagationFrame,
            const std::vector< boost::function< Eigen::Vector3d( ) > > occultingBodyPositions =
            std::vector< boost::function< Eigen::Vector3d( ) > >( ),
            const std::vector< double > occultingBodyRadii = std::vector< double > ( ),
            const double sourceRadius = 0.0 ):
...
private:
    ...
    std::vector< boost::function< Eigen::Vector3d( ) > > localFrameSurfaceNormals_;
    ...
    boost::function< Eigen::Quaterniond( ) > rotationFromLocalToPropagationFrame_;
