...
BOOST_AUTO_TEST_CASE( testSimpleGeometryPanelledRadiationPressure )
{
    ...
    bodyMap[ "Vehicle" ]->setRotationalEphemeris(
                boost::make_shared< ConstantRotationalEphemeris >(
    ...
    for( unsigned int test = 0; test < 2; test++ )
    {
        ...
        std::vector< double > testTimes = boost::assign::list_of( 0.0 )( orbitalPeriod / 4.0 )
                ( orbitalPeriod / 2.0 )( 3.0 * orbitalPeriod / 4.0 );
        ...
        for( unsigned int i = 0; i < testTimes.size( ); i++ )
        {
            ...
            {
                boost::shared_ptr< PanelledRadiationPressureAcceleration > panelledRadiationPressureAcceleration =
                        boost::dynamic_pointer_cast< PanelledRadiationPressureAcceleration >( accelerationModel );
