    ...
    massFunction_( massFunction )
{
    sourcePositionFunction_ =
            boost::bind( &RadiationPressureInterface::getCurrentSolarVector, radiationPressureInterface );
    acceleratedBodyPositionFunction_ = boost::lambda::constant( Eigen::Vector3d::Zero( ) );
    radiationPressureFunction_ = boost::bind( &RadiationPressureInterface::getCurrentRadiationPressure, radiationPressureInterface );
    ...
    for( int i = 0; i < radiationPressureInterface->getNumberOfPanels( ); i++ )
    {
        panelEmmisivittyFunctions_.push_back(
                    boost::bind( &PanelledRadiationPressureInterface::getEmmisivity, radiationPressureInterface, i ) );
        panelDiffuseReflectionCoefficientFunctions_.push_back(
                    boost::bind( &PanelledRadiationPressureInterface::getDiffuseReflectionCoefficient, radiationPressureInterface, i ) );
        panelSurfaceNormalFunctions_.push_back(
                    boost::bind( &PanelledRadiationPressureInterface::getCurrentSurfaceNormal, radiationPressureInterface, i ) );
        panelAreaFunctions_.push_back(
                    boost::bind( &PanelledRadiationPressureInterface::getArea, radiationPressureInterface, i ) );
    }
