...
public:
    ...
    VelocityObervationPartial(
            const boost::shared_ptr< VelocityObservationScaling > velocityObservationScaler,
            const std::map< observation_models::LinkEndType, boost::shared_ptr< CartesianStatePartial > >& velocityPartialList,
            const estimatable_parameters::EstimatebleParameterIdentifier parameterIdentifier ):
...
protected:
    ...
    boost::shared_ptr< VelocityObservationScaling > velocityObservationScaler_;

    //! List of velocity partial per link end.
    std::map< observation_models::LinkEndType, boost::shared_ptr< CartesianStatePartial > > velocityPartialList_;

    //! Iterator over list of velocity partial per link end (predeclared for efficiency).
    std::map< observation_models::LinkEndType, boost::shared_ptr< CartesianStatePartial > >::iterator velocityPartialIterator_;
