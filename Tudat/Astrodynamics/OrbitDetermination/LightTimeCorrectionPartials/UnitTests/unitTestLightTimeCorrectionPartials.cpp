...
BOOST_AUTO_TEST_CASE( testOneWayRangePartials )
{
    ...
    for( unsigned int i = 0; i < doubleParameterVector.size( ); i++ )
    {
        std::cout<<"Parameter id: "<<doubleParameterVector.at( i )->getParameterName( ).first<<" "<<
                   doubleParameterVector.at( i )->getParameterName( ).second.first<<std::endl;
    }
         ...
         linkEndIterator++ )
    {
        ...
        {
            std::cout<<"Double parameter "<<i<<" "<<analyticalObservationPartials.size( )<<" "<<fullAnalyticalPartialSet.first.size( )<<" "<<
                       doubleParameterVector.size( )<<std::endl;
            ...
            std::cout<<currentParameterPartial<<" "<<-1.0 * numericalPartialsWrtDoubleParameters[ i ]<<std::endl;
