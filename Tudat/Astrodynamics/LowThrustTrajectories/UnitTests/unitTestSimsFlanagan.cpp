...
BOOST_AUTO_TEST_CASE( test_Sims_Flanagan_full_propagation )
{
    ...
    for ( std::map< double, Eigen::Vector6d >::iterator itr = trajectory.begin( ) ; itr != trajectory.end( ) ; itr++ )
    {
        ...
        std::cout<<"Masses "<<massProfile[ itr->first ][ 0 ]<<" "<<currentExpectedMass<<std::endl;
        std::cout<<"Masses "<<calculatedMass<<" "<<currentExpectedMass<<std::endl;
        std::cout<<"Accelerations "<<std::endl<<thrustAccelerationProfile[ itr->first ].transpose( )<<std::endl<<
                   currentExpectedThrustAcceleration.transpose( )<<std::endl;
        std::cout<<"Accelerations "<<std::endl<<calculatedThrustAcceleration.transpose( )<<std::endl<<
                   currentExpectedThrustAcceleration.transpose( )<<std::endl;
        std::cout<<"Forces "<<std::endl<<calculatedThrust.transpose( )<<std::endl<<
                   currentExpectedThrust.transpose( )<<std::endl;
