...
BOOST_AUTO_TEST_CASE( testMutualSphericalHarmonicGravity )
{
    ...
    double finalTime = 2.0 * physical_constants::JULIAN_YEAR;
    ...
    intialKeplerElements = integrationResultWithDissipationInIoKepler.begin( )->second;
    meanMotion = basic_astrodynamics::computeKeplerMeanMotion(
                intialKeplerElements( 0 ), getBodyGravitationalParameter( "Jupiter" ) + getBodyGravitationalParameter( "Io" ) );
    orbitalPeriod = 2.0 * mathematical_constants::PI / meanMotion;
    //    double jupiterForcingTime = 9.925 * 3600.0 * orbitalPeriod / ( 2.0 * std::fabs( orbitalPeriod - 9.925 * 3600.0 ) );
    //    double jupiterQualityFactor = 1.0 / std::sin( 2.0 * mathematical_constants::PI * jupiterTimeLag / ( jupiterForcingTime ) );

    double ioQualityFactor = 1.0 / std::sin( 2.0 * mathematical_constants::PI * ioTimeLag / orbitalPeriod );
    ...
    tudat::input_output::writeMatrixToFile( tudat::utilities::convertStlVectorToEigenVector(
                                                tudat::utilities::createVectorFromMapValues( semiMajorAxesIoTide ) ), "ioSemiMajorAxis.dat" );
    tudat::input_output::writeMatrixToFile( tudat::utilities::convertStlVectorToEigenVector(
                                                tudat::utilities::createVectorFromMapValues( eccentricitiesIoTide ) ), "ioEccentricity.dat" );
    tudat::input_output::writeMatrixToFile( tudat::utilities::convertStlVectorToEigenVector(
                                                tudat::utilities::createVectorFromMapKeys( semiMajorAxesIoTide ) ), "ioTimes.dat" );

    std::cout<<"Io values: "<<ioLoveNumber<<" "<< ioLoveNumber / ioQualityFactor<<std::endl;
    ...
    BOOST_CHECK_CLOSE_FRACTION( semiMajorAxisFitIoTide.at( 1 ), theoreticalSemiMajorAxisRateFromIoTide, 1.0E-3 );
    BOOST_CHECK_CLOSE_FRACTION( eccentricityFitIoTide.at( 1 ), theoreticaEccentricityRateFromIoTide, 1.0E-3 );
