...
BOOST_AUTO_TEST_CASE( testPeriodicGravityFieldVariations )
{
    ...
    getNominalJupiterGravityField( nominalCosineCoefficients, nominalSineCoefficients );
    {
        ...
        for( int k = 0; k < 4; k++ )
        {
            ...
            if( k < 3 )
            {
                variationSettings = std::make_shared< PeriodicGravityFieldVariationsSettings >(
            ...
            {
                variationSettings = std::make_shared< PeriodicGravityFieldVariationsSettings >(
                            std::vector< Eigen::MatrixXd >( { cosineAmplitudes.at( 0 ) } ),
                            std::vector< Eigen::MatrixXd >( { sineAmplitudes.at( 0 ) } ),
                            std::vector< double >( { frequencies.at( 0 ) } ),
                            std::vector< double >( { 0.0 } ), referenceEpoch,
                            minimumDegree, minimumOrder );
            }
            ...
            NamedBodyMap dummySystem;
            dummySystem[ "Jupiter" ] = std::make_shared< Body >( );
            dummySystem.at( "Jupiter" )->setGravityFieldModel( timeDependentGravityField );
