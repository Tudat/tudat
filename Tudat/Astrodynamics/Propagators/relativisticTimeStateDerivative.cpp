#include "Astrodynamics/Propagators/relativisticTimeStateDerivative.h"

namespace tudat
{

void DirectProperTimeRateStateDerivative::updateStateDerivativeModel( const double currentTime )
{
...
SecondOrderBarycentricToBodyCentricTimeStateDerivative::SecondOrderBarycentricToBodyCentricTimeStateDerivative(
        const std::map< std::string, boost::shared_ptr< bodies::Body > > bodyMap, const std::string centralBody,
        const std::vector< std::string >& externalBodies,
        const std::map< std::string, std::pair< int, int > > sphericalHarmonicGravityExpansions,
        const boost::function< double( const double ) > timeVariableConversionFunction,
        const double distanceScalingFactor,
        const std::vector< std::string >& angularMomentumBodies ): FirstOrderBarycentricToBodyCentricTimeStateDerivative(
        bodyMap, centralBody, externalBodies, sphericalHarmonicGravityExpansions, timeVariableConversionFunction,
        distanceScalingFactor, propagators::second_order_barycentric_to_bodycentric )
{
    for( unsigned int i = 0; i < angularMomentumBodies.size( ); i++ )
    {
        ...
        {
            angularMomentumFunctions_[ i ] = boost::lambda::constant( ( Eigen::Vector3d( )<<0.0, 0.0, 1.92E41 ).finished( ) );
        }
        else if( externalBodies.at( i ) == "Jupiter" )
        {
            angularMomentumFunctions_[ i ] = boost::lambda::constant( ( Eigen::Vector3d( )<<0.0, 0.0, 6.9E38 ).finished( ) );
        }
        else if( externalBodies.at( i ) == "Saturn" )
        {
            angularMomentumFunctions_[ i ] = boost::lambda::constant( ( Eigen::Vector3d( )<<0.0, 0.0, 1.42E38 ).finished( ) );
        }
        //boost::shared_ptr< bodies::CelestialBody > celestialBody =
        //        boost::dynamic_pointer_cast< bodies::CelestialBody >( bodyMap.at( externalBodies.at( i ) ) );
        //externalBodyStateFunctions_.push_back( boost::bind( &bodies::Body::getStateInBaseFrameFromEphemeris, bodyMap.at( externalBodies.at( i ) ), _1 ) );
    }
    currentBodySpeeds_.resize( externalBodyStateFunctions_.size( ) );
}


Eigen::Matrix< double, Eigen::Dynamic, 1 > SecondOrderBarycentricToBodyCentricTimeStateDerivative::calculateSystemStateDerivative(
        const double currentGlobalTime, const Eigen::Matrix< double, Eigen::Dynamic, 1 >& integratedValue )
{
    //updateStateDerivativeModel( currentGlobalTime );
    return ( Eigen::Matrix< double, 1, 1 >( ) << calculateFirstOrderTcbToTcgIntegrand( currentVelocity_, currentExternalPotential_ ) +
             calculateSecondOrderTcbToTcgIntegrand(
                 currentVelocity_, currentExternalPotential_, currentCentralBodyState_.segment( 3, 3 ), currentExternalVectorPotential_,
                 currentSecondOrderExternalPotentialCorrection_ ) ).finished( );
}

void SecondOrderBarycentricToBodyCentricTimeStateDerivative::updateStateDerivativeModel( const double baseFrameTime )
{
    updateBaseVariables( baseFrameTime );

    for( std::map< int, boost::function< Eigen::Vector3d( ) > >::iterator momentaIterator = angularMomentumFunctions_.begin( );
    ...
    for( unsigned int i = 0; i < externalBodyStateFunctions_.size( ); i++ )
    {
        ...
        currentBodySpeeds_[ i ] = currentExternalBodyStates_[ i ].segment( 3, 3 ).norm( );
    }

    currentExternalPotential_= calculateFirstOrderExternalScalarPotential(
                currentExternalBodyGravitationalParameters_, currentExternalBodyDistances_);//, currentBodySpeeds_, currentExternalBodyStates_ );

    currentSecondOrderExternalPotentialCorrection_ = calculateSecondOrderExternalScalarPotentialCorrection(
                currentExternalBodyGravitationalParameters_, currentExternalBodyDistances_, currentBodySpeeds_, currentExternalBodyStates_,
                currentCentralBodyState_.segment( 3, 3 ) );
    currentExternalVectorPotential_ = calculateExternalVectorPotential( currentExternalBodyGravitationalParameters_, currentExternalBodyDistances_,
                                                                        currentExternalBodyStates_, currentAngularMomenta_ );
}
