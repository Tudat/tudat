...
#include "Astrodynamics/Propagators/environmentUpdater.h"
#include "Astrodynamics/Propagators/singleStateTypeDerivative.h"
#include "Astrodynamics/Relativity/relativisticPotentials.h"
#include "Astrodynamics/Relativity/Metrics/metric.h"
#include "Astrodynamics/Relativity/relativisticTimeConversions.h"
#include "Astrodynamics/Relativity/relativisticEquationsOfMotion.h"
#include "Mathematics/NumericalIntegrators/createNumericalIntegrator.h"
...
public:
    ...
    RelativisticTimeStateDerivative(
        ...
        propagators::SingleStateTypeDerivative< double, double >( propagators::proper_time ),
...
class DirectProperTimeRateStateDerivative: public RelativisticTimeStateDerivative
{
public:
    DirectProperTimeRateStateDerivative(
            const boost::shared_ptr< relativity::Metric > spaceTimeMetric,
            const std::pair< std::string, std::string > referencePointId,
            const NamedBodyMap bodyMap ):RelativisticTimeStateDerivative( referencePointId, propagators::direct_from_metric ),
        spaceTimeMetric_( spaceTimeMetric )
    {
        if( bodyMap.count( referencePointId.first ) == 0 )
        {
        ...
        else
        {
            ...
            {
                if( boost::dynamic_pointer_cast< bodies::CelestialBody >( bodyMap.at( referencePointId.first  ) ) != NULL )
                {
                ...
                referencePointStateFunction_ = boost::bind( &bodies::Body::getState, bodyMap.at( referencePointId.first ) );

            }
            else
            {
                referencePointStateFunction_ = ephemerides::getLinkEndCompleteCurrentStateFunction< double >(
                            referencePointId, bodyMap );
            }
        }
    }

    Eigen::Matrix< double, Eigen::Dynamic, 1 > calculateSystemStateDerivative(
            const double time, const Eigen::Matrix< double, Eigen::Dynamic, 1 >& stateOfSystemToBeIntegrated )
    {
        return ( Eigen::Matrix< double, Eigen::Dynamic, 1 >( 1, 1 ) << currentProperTimeDerivative_ ).finished( );
    }

    void updateStateDerivativeModel( const double currentTime );

    boost::shared_ptr< relativity::Metric > getSpaceTimeMetric( )
    {
...
protected:
    boost::shared_ptr< relativity::Metric > spaceTimeMetric_;

    std::pair< std::string, std::string > referencePointId_;

    boost::function< basic_mathematics::Vector6d( ) > referencePointStateFunction_;
...
class SecondOrderBarycentricToBodyCentricTimeStateDerivative: public FirstOrderBarycentricToBodyCentricTimeStateDerivative
{
public:
...
private:
    ...
    std::map< int, boost::function< Eigen::Vector3d( ) > > angularMomentumFunctions_;
