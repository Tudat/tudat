        ...
        const bool propagateVariationalEquations = 1 )
{
    ...
    Eigen::Quaterniond noRotationQuaternion = Eigen::Quaterniond( Eigen::AngleAxisd( 1.0E-9, Eigen::Vector3d::UnitZ( ) ) *
                                                                  Eigen::AngleAxisd( 2.0E-9, Eigen::Vector3d::UnitX( ) ) *
                                                                  Eigen::AngleAxisd( -0.5E-9, Eigen::Vector3d::UnitZ( ) ) );
    ...
    unitRotationState( 4 ) = 1.0E-8;
    unitRotationState( 5 ) = -1.0E-8;
    unitRotationState( 6 ) = std::sqrt( tudat::spice_interface::getBodyGravitationalParameter( "Mars") /
                                        std::pow( phobosSemiMajorAxis, 3.0 ) ) + 1.0E-8;

    Eigen::Matrix< double, 7, 1 > originalRotationState = unitRotationState;
    unitRotationState += initialStateDifference;
    unitRotationState.segment( 0, 4 ).normalize( );
    std::cout<<"State diff: "<<( unitRotationState - originalRotationState ).transpose( )<<std::endl;
...
//                boost::make_shared< TorqueSettings >( second_order_gravitational_torque ) );
...

    {
        ...
        Eigen::Matrix< StateScalarType, Eigen::Dynamic, 1 > testStates =
                Eigen::Matrix< StateScalarType, Eigen::Dynamic, 1 >::Zero( 7 );
...
BOOST_AUTO_TEST_CASE( testPhobosRotationVariationalEquationCalculation )
{
    ...
    // Define state perturbation
    statePerturbation = 100.0 * ( Eigen::Matrix< double, 7, 1>( ) <<
                          1.0E-8, 1.0E-8, 1.0E-8, 1.0E-8, 1.0E-10, 1.0E-10, 1.0E-10 ).finished( );
    ...
    for( unsigned int j = 0; j < 7; j++ )
    {
        ...
        Eigen::VectorXd stateDifferenceUp = upPerturbedState - currentOutput.second.at( 0 );
        stateDifferenceUp.segment( 0, 4 ).normalize( );
        std::cout<<std::endl<<"Up"<<j<<" "<<( stateDifferenceUp ).transpose( )<<std::endl;
        std::cout<<"Up"<<j<<" "<<( perturbedState ).transpose( )<<std::endl;
        ...
        Eigen::VectorXd stateDifferenceDown = downPerturbedState - currentOutput.second.at( 0 );
        stateDifferenceDown.segment( 0, 4 ).normalize( );
        std::cout<<std::endl<<"Down"<<j<<" "<<( stateDifferenceDown ).transpose( )<<std::endl;
        std::cout<<"Down"<<j<<" "<<( perturbedState ).transpose( )<<std::endl;

        manualPartial.block( 0, j, 7, 1 ) =
                ( upPerturbedState.segment( 0, 7 ) - downPerturbedState.segment( 0, 7 ) ) / ( 2.0 * statePerturbation( j ) );
        std::cout<<std::endl<<"Diff"<<j<<" "<<( upPerturbedState - downPerturbedState ).transpose( )<<std::endl;

    }

    std::cout<<manualPartial<<std::endl<<std::endl<<
               stateTransitionAndSensitivityMatrixAtEpoch<<std::endl<<std::endl<<
               ( manualPartial ).cwiseQuotient(
