        ...
        const bool propagateVariationalEquations = 1 )
{
    ...
    appliedStateDifference = unitRotationState - originalRotationState ;
...
BOOST_AUTO_TEST_CASE( testPhobosRotationVariationalEquationCalculation )
{
    ...
    // Compute state transition and sensitivity matrices
    Eigen::Vector7d appliedStateDifference;
    currentOutput = executePhobosRotationSimulation< double, double >(
                Eigen::Matrix< double, 7, 1 >::Zero( ), appliedStateDifference );
    Eigen::MatrixXd stateTransitionAndSensitivityMatrixAtEpoch = currentOutput.first.at( 0 );
    Eigen::VectorXd nominalState = currentOutput.second.at( 0 );

    // Define state perturbation
    statePerturbation = 1.0E-4 * ( Eigen::Matrix< double, 7, 1>( ) <<
    ...
    {
        Eigen::Vector7d appliedStateDifferenceUp, appliedStateDifferenceDown;
...
//                                              stateDifferenceUp ) ).cwiseQuotient( stateDifferenceUp ).transpose( )<<std::endl;
    ...
    for( unsigned int j = 4; j < 7; j++ )
    {
        ...
        downPerturbedState = executePhobosRotationSimulation< double, double >(
                    perturbedState, appliedStateDifferenceDown, Eigen::Matrix< double, 12, 1 >::Zero( ), 0 ).second.at( 0 );
    ...
    TUDAT_CHECK_MATRIX_CLOSE_FRACTION(
                ( manualPartial.block( 0, 4, 7, 3 ) ), ( stateTransitionAndSensitivityMatrixAtEpoch.block( 0, 4, 7, 3 ) ), 1.0E-5 );

//    std::cout<<manualPartial.block( 0, 4, 7, 3 )<<std::endl<<std::endl
//             <<stateTransitionAndSensitivityMatrixAtEpoch.block( 0, 4, 7, 3 )<<std::endl<<std::endl<<
