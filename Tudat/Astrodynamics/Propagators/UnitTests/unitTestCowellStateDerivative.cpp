...
BOOST_AUTO_TEST_CASE( testCowellPopagatorCentralBodies )
{
    ...
    boost::shared_ptr< IntegratorSettings< > > integratorSettings =
            ...
            ( rungeKutta4, initialEphemerisTime, finalEphemerisTime, 250.0 );
    ...
    std::map< double, Eigen::VectorXd > numericalSolution1;
    std::map< double, Eigen::VectorXd > numericalSolution2;
...
BOOST_AUTO_TEST_CASE( testCowellPopagatorKeplerCompare )
{
    ...
    while( currentTime < finalEphemerisTime - buffer )
    {
        basic_mathematics::Vector6d stateDifference = orbital_element_conversions::convertKeplerianToCartesianElements(
                    propagateKeplerOrbit( initialKeplerElements, currentTime - initialEphemerisTime, earthGravitationalParameter ),
                    earthGravitationalParameter ) - moonEphemeris->getCartesianStateFromEphemeris( currentTime );
        for( int i = 0; i < 3; i++ )
        {
            BOOST_CHECK_SMALL( stateDifference( i ), 0.02 );
