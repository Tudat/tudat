...
public:
            ...
            Eigen::Block< Eigen::Matrix< StateScalarType, Eigen::Dynamic, Eigen::Dynamic > > stateDerivative )
    {
        ...
        for( unsigned int i = 0; i < torquesActingOnBodies.size( ); i++ )
        {
            ...
            std::cout << "MRP: " << currentModifiedRodriguesParameters.transpose( ) << std::endl;
                           ...
                           const int startRow )
    {
        ...
        for( unsigned int i = 0; i < this->bodiesToPropagate_.size( ); i++ )
        {
            ...
            modifiedRodriguesParametersVector = unprocessedState.block( startRow + i * 7, 0, 3, 1 );
            modifiedRodriguesParametersMagnitude = modifiedRodriguesParametersVector.norm( );
            if ( modifiedRodriguesParametersMagnitude >= 1.0 )
            {
                // Invert flag
                unprocessedState( startRow + i * 7 + 3 ) = std::fabs( unprocessedState( startRow + i * 7 + 3 ) - 1.0 );
                ...
                unprocessedState.segment( startRow + i * 7, 3 ) = modifiedRodriguesParametersVector;
            }
        }
    }
