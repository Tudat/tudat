...
;
        ...
        else if( testCase == 1 )
        {
                 ...
                 outputIterator != dependentVariableResult.end( ); outputIterator++ )
            {
                ...
                //                std::cout<<std::setprecision( 16 )<<( reference_frames::getVelocityBasedLvlhToInertialRotation(
                //                               outputIterator->second.segment( 3, 6 ), basic_mathematics::Vector6d::Zero( ) )
                //                            * thrustInterpolator->interpolate( outputIterator->first  ) ).transpose( )<<std::endl;
                //                std::cout<<outputIterator->second.segment( 0, 3 ).transpose( )<<std::endl;
                //                std::cout<<( reference_frames::getVelocityBasedLvlhToInertialRotation(
                //                               outputIterator->second.segment( 3, 6 ), basic_mathematics::Vector6d::Zero( ) )
                //                            * thrustInterpolator->interpolate( outputIterator->first  ) ).norm( )<<std::endl;
                //                std::cout<<outputIterator->second.segment( 0, 3 ).norm( )<<std::endl;

                std::cout<<"Rotation in output: "<<std::endl<<std::setprecision( 16 )<<
                           reference_frames::getVelocityBasedLvlhToInertialRotation(
                               outputIterator->second.segment( 3, 6 ), basic_mathematics::Vector6d::Zero( ) )<<std::endl;
                std::cout<<"State in output "<<( integrationResult.at( outputIterator->first )  ).transpose( )<<std::endl<<std::endl;
