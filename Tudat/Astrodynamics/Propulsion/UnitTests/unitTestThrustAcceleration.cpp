...
int main( )
{
    ...
    apolloSphericalEntryState( SphericalOrbitalStateElementIndices::radiusIndex ) = spice_interface::getAverageRadius( "Earth" ) + 4.0E3;
    ...
    apolloSphericalEntryState( SphericalOrbitalStateElementIndices::speedIndex ) = 2.0E3;
    ...
    double currentThrustForce, currentMassRate, expectedThrust;
    std::vector< double > currentThrustInput;

    std::cout<<numericalSolution.size( )<<" "<<dependentVariableSolution.size( )<<std::endl;
         ...
         variableIterator != dependentVariableSolution.end( ); variableIterator++ )
    {
        ...
        std::cout<<currentDynamicPressure<<" "<<currentMachNumber<<" "<<
                   currentThrustForce<<" "<<expectedThrust - currentThrustForce<<std::endl;

        //BOOST_CHECK_CLOSE_FRACTION( expectedThrust, currentThrustForce, std::numeric_limits< double >::epsilon( ) );
    }
}
