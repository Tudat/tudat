...
public:
    ...
    void resetDependentValues( const std::vector< DependentVariableType >&  dependentValues )
    {
        ...
        {
            std::cerr<<"Error when resetting dependent values for piecewise constant interpolator, sizes are inconsistent "<<
                    dependentValues.size( )<<" "<<dependentValues_.size( )<<std::endl;
        }
