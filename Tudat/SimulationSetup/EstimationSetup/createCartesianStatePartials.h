 ...
 *  \return Single object that computes partial of given observable w.r.t. given parameter.
 */
boost::shared_ptr< VelocityObervationPartial > createVelocityObservablePartialWrtVelocity(
        const observation_models::LinkEnds linkEnds,
        const simulation_setup::NamedBodyMap& bodyMap,
        const std::string bodyToEstimate,
        const boost::shared_ptr< VelocityObservationScaling > velocityObservableScaler );
...

{
    // Create scaling object, to be used for each partial created here (i.e. same scaling for different parameters but same
    // observable).
    boost::shared_ptr< VelocityObservationScaling > velocityObservableScaling =
            boost::make_shared< VelocityObservationScaling >( );
    ...
    for( unsigned int i = 0; i < initialDynamicalParameters.size( ); i++ )
    {
                    ...
                    initialDynamicalParameters.at( i ) ) != NULL )
        {
            ...
            // Create partial (if needed)
            boost::shared_ptr< VelocityObervationPartial > currentObservablePartial =
...
{
    std::map< observation_models::LinkEnds, std::pair< SingleLinkObservationThreePartialList,
            boost::shared_ptr< PositionPartialScaling > > > velocityObservablePartials;
