...
BOOST_AUTO_TEST_CASE( testPolyhedronUtilities )
{
    ...
    double computedArea = polyhedron_utilities::computeSurfaceArea(verticesCoordinates, verticesDefiningEachFacet);
    BOOST_CHECK_CLOSE_FRACTION( expectedArea, computedArea, tolerance );

    // Computation of volume
    double expectedVolume = l * w * h;
    double computedVolume = polyhedron_utilities::computeVolume(verticesCoordinates, verticesDefiningEachFacet);
    ...
    Eigen::Vector3d computedCentroid = polyhedron_utilities::computeCentroidPosition(verticesCoordinates, verticesDefiningEachFacet);
    ...
    Eigen::MatrixXd correctedVerticesCoordinates = polyhedron_utilities::modifyCentroidPosition(
            verticesCoordinates, verticesDefiningEachFacet, desiredCentroid);
    computedCentroid = polyhedron_utilities::computeCentroidPosition(correctedVerticesCoordinates, verticesDefiningEachFacet);
    ...
    Eigen::Matrix3d computedInertiaTensor = polyhedron_utilities::computeInertiaTensor(
