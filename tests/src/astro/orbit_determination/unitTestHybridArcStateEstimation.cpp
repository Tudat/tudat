...
BOOST_AUTO_TEST_CASE( test_HybridArcStateEstimation )
{
    ...
    // Test error range: 10 m in-plane position and 10 micron/s in-plane velocity for Mars
    for( unsigned int j = 0; j < 2; j++ )
    {
        BOOST_CHECK_SMALL( std::fabs( parameterError( j ) ), 10.0 );
        BOOST_CHECK_SMALL( std::fabs( parameterError( j + 3 ) ), 10.0E-6  );
    }

    // Test error range: 2000 m in-plane position and 2.0 mm/s in-plane velocity for Mars (poor values due to short arc)
    BOOST_CHECK_SMALL( std::fabs( parameterError( 2 ) ), 2000.0 );
    BOOST_CHECK_SMALL( std::fabs( parameterError( 5 ) ), 2.0E-3  );
    ...
    BOOST_CHECK_SMALL( std::fabs( parameterError( parameterError.rows( ) - 2 ) ), 5.0E11 );
    BOOST_CHECK_SMALL( std::fabs( parameterError( parameterError.rows( ) - 1 ) ), 5.0E6 );
