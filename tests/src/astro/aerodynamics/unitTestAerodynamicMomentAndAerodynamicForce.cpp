...
Eigen::Vector3d massDependentCenterOfMass( const double mass )
{
    ...
    std::cout<<"New com "<<com - ( Eigen::Vector3d( ) <<  -0.6624, 0.0, 0.1369 ).finished( )<<std::endl;
    return com;
}
...
BOOST_AUTO_TEST_CASE( testCombinedAerodynamicForceAndMoment )
{
    ...
    for( unsigned propagationType = 1; propagationType < 2 ; propagationType++ )
    {
        for( unsigned int i = 0; i < 1; i++ )
        {
            ...
            std::cout<<"Reference "<<aerodynamicCoefficients->getMomentReferencePoint( ).transpose( )<<std::endl;
            if( i == 1 )
            {
...
//                                "Earth", "ECLIPJ2000", "VehicleFixed", &sideslipBankAngleFunction ),
            ...
            auto terminationSettings = std::make_shared< propagators::PropagationTimeTerminationSettings >( fixedStepSize );
            std::shared_ptr< TranslationalStatePropagatorSettings < double > > translationalPropagatorSettings =
                    std::make_shared< TranslationalStatePropagatorSettings< double > >(
                        centralBodies, accelerationModelMap, bodiesToPropagate, systemInitialState, simulationStartEpoch, integratorSettings,
...
//                bodies.at( "Apollo" )->getMassProperties( )->setCurrentMass( mass );
//                bodies.at( "Apollo" )->getMassProperties( )->updateMassDistribution( it.first );
//                bodies.at( "Apollo" )->setState( stateOutput.at( it.first ).segment( 0, 6 ) );
//                bodies.at( "Apollo" )-
                aerodynamicCoefficientInterface->updateFullCurrentCoefficients(
                { machNumber, angleOfAttack, sideslipAngle } );
                ...
                Eigen::Vector3d bodyFixedMomentCoefficients =
                        rotationFromAerodynamicToBodyFrame  * momentCoefficients;
                Eigen::Vector3d inertialForceCoefficients =
                    (  ( i == 0 ) ? rotationFromAerodynamicFrame : rotationFromBodyFrame ) * forceCoefficients;
                ...
                Eigen::Vector3d testForceContribution = (
                    ( rotationFromAerodynamicToBodyFrame.transpose( ) * ( aerodynamicCoefficients->getMomentReferencePoint( ) - centerOfMass ) )
                ...
                std::cout<<"Force diff. "<<( directMomentContribution ).transpose( )<<std::endl;
                std::cout<<testForceContribution.transpose( )<<std::endl;

                std::cout<<"Norms "<<( testMomentCoefficientsWithoutForceContribution - testMomentCoefficients ).norm( )<<" "<<
                       testForceContribution.norm( )<<std::endl;
                ...
                std::cout<<( aerodynamicForce.cwiseQuotient( testAerodynamicForce ) ).transpose( )<<std::endl;
                std::cout<<aerodynamicForce.transpose( )<<std::endl;
                std::cout<<testAerodynamicForce.transpose( )<<std::endl<<std::endl;
                std::cout<<aerodynamicMoment.norm( ) / testAerodynamicMoment.norm( )<<std::endl;
                std::cout<<( aerodynamicMoment.cwiseQuotient( testAerodynamicMoment ) ).transpose( )<<std::endl;
                std::cout<<aerodynamicMoment.transpose( )<<std::endl;
                std::cout<<testAerodynamicMoment.transpose( )<<std::endl<<std::endl<<std::endl;
            }
        }
    }
}
