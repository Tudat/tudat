...
Each of the referenced JSON files is parsed individually using the function :literal:`json readJSON( const path& filePath, ... )` declared in :class:`Tudat/External/JsonInterface/Support/modular.h`. In addition to parsing the contents of the file at :literal:`filePath` using the :literal:`json::parse` function, this function adds the following features:
