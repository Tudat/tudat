...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Welcome to the troubleshooting guide. You have most likely been asked to come here because you have a problem with your Tudat installation (or stumbled upon this page, great, yay!).
How to use this guide: please complete all checks and copy the output (even if a check seems succesfull). Some checks will provide additional tests and solutions in case the check initially fails. Please complete these instructions as well and redo the original test!

1.1 Check CMake
Open your terminal emulator and type
cmake --version

Report the full output of this command.
If your output looks like the example below following, you can assume that CMake has been installed correctly.
cmake version 3.4.3  
CMake suite maintained and supported by Kitware (kitware.com/cmake).

1.2 Check Compiler
Open your terminal and check the version of your C++ compiler using the command below. This command is follow by two example outputs. Both outputs are fine, but if you get compilation errors, please report the versions.
...
Output command 1 example 1 (GCC)
Output command 2 example 2 (Clang)
...
If so try uninstall and reinstalling the latest version, please follow the Installation_Guide to make sure you don't miss any steps.
...
2.1. Boost compiler compatability
...
MinGW 4.9.2           ✓             ✓              ✓
MinGW 5.3             ✗             ✓              ✓
2.2 Check if Boost build succesfully
...
If the automated process fails, it is necessary to take note of where it fails please copy the output of CMake for specifics.
...
Make logs of bootstrap and b2 command

2.5 Check QtCreator CMake binary
...
Make a screenshot of the CMake tab if problems persist.

2.6 Check QtCreator Kits
...
Make a screenshot of the Kits tab if problems persist.
...
2. Type terminal
 
3. Confirm your choice with Enter
...
cd /home/John
cd tudatBundle
Should be everything you need to get to the right place.
Note that for Windows you can't cd to another drive (e.g. from C:\ to D:\). To switch drives, simply enter D:. Also note that for Windows directories are seperated by a backslash (\) and UNIX-like systems follow POSIX's forward-slash seperator ("/"). However, Windows is POSIX compliant, meaning that cd \Users\John and cd /Users/John are both correct on Windows.
A.2 Creating log files
...
A.4 Modifying the PATH variable
