...
Other relevant properties of the satellite are set constant:
  ...
  - Mass of 400 kg.
  - Reference area of 10 :math:`\text{m}^2`.
  - Drag coefficient of 2.2.
  - Radiation pressure coefficients of 1.5.

The following orbital perturbations are considered:

  - Atmospheric drag (NRLMSISE00 model).
  - Zonal terms of the geopotential up to degree 7 (GGM02C model).
  - Sun and Moon's third-body gravitational attraction.
...
The global frame origin is the Solar system's barycentre, which is the default value for the key :jsonkey:`globalFrameOrigin`, so there is no need to provide it. The :jsonkey:`globalFrameOrientation` is set to :literal:`"J2000"`.
...
We also specify the list of variables to be exported. Only the initial and final epoch and perigee altitude are relevant. The epoch is outputted in the first column of the generated files automatically (unless :jsonkey:`epochsInFirstColumn` is set to :literal:`false`). We request to output only the results of the initial and final epochs by settings :jsonkey:`onlyInitialStep` and :jsonkey:`onlyFinalStep` to :literal:`true` (note that these keys are not mutually exclusive, i.e. settings both to :literal:`true` will result in a results matrix with two rows corresponding to the initial and final epochs). The variable to be outputted is the satellite's periapsis altitude w.r.t. Earth.
