...
:jsontype:`T[]` denotes that an array of :jsontype:`T` is expected. If no number is indicated, there is no constraint on the size of the array. For instance, :jsontype:`number[6]` is used for state vectors. A second :jsontype:`[]` is used to represent matrices, e.g. :jsontype:`number[][]` will be converted to :class:`Eigen::MatrixXd`, and :jsontype:`number[3][3]` will be converted to :class:`Eigen::Matrix3d`.
